...
const filteredData = [
  ...
  },
  {
    ...
    label: 'menu.school-list',
    to: `${adminRoot}/school-list`,
...
const data = filteredData.filter((tab) => {
  if (userRole === 'Super Admin') {
    // Show all tabs for Super Admin
    return true;
  }
  if (tab.roles && tab.roles.includes(userRole)) {
    // Show the tab if user has a role specified for that tab
    return true;
  }
  // Default: Hide the tab if no specific conditions are met
  return false;
});
console.log(data);
